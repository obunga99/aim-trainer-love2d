...
function hitscan(x, y, key, distance) --method for scanning whenever the mouse clicks one of the targets 
		...
		if key == 1 and x > target.x - target.w and x < target.x + target.w and y > target.y - target.w and y < target.y + target.h then --checks the collision with the mouse and a target through aab
		...
		if key == 1 and x < target.x - target.w and x > target.x + target.w and y < target.y and y > target.y + target.h then
